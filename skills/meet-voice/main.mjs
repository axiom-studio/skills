import { joinMeet, meetingURL } from './meet.mjs';
import { audioCommands, openAudio, SAMPLE_RATE } from './audio.mjs';
import { CortexConversation, decodeSpeech, SpeechClient, speechChunks, UtteranceDetector } from './bridge.mjs';
...
async function main() {
  ...
  const speech = new SpeechClient(config.speech);
  ...
  try {
    ...
    meeting = await joinMeet({ url: config.meetURL, profileDir: config.profileDir, executablePath: config.executablePath,
      displayName: config.displayName, signal: controller.signal,
      onAdmissionRequested: () => {
        process.send?.({ status: 'awaiting_admission' });
        void conversation.postStatus('I requested to join the Google Meet and am waiting for the host to admit me.',
    ...
    presenceCheck = setInterval(async () => {
      try {
        const present = await meeting.page.getByRole('button', { name: /^(Leave call|Leave meeting)$/i }).isVisible();
        if (!present) stop();
      } catch { stop(); }
    }, 5000);
    presenceCheck.unref();
    ...
    try {
      await conversation.postStatus('I joined the Google Meet and am listening. Ask me to leave in this Seal Chat when you are done.', controller.signal);
    } catch (error) {
    ...
    await playText('Hello, I am the Axiom voice assistant. I am listening and can respond to requests.');
    ...
    if (!controller.signal.aborted) {
      try {
        await conversation.postStatus('The Google Meet voice session failed. Check the meeting link, host admission, and audio setup before starting it again.');
      } catch (statusError) {
